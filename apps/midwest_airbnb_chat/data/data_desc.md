...
| `review_scores_rating` | numeric | Average overall rating score for the listing on a 0 to 5 scale (contains `NA` for unrated listings with no reviews). |
| `reviews_per_month` | numeric | The average number of reviews per month the listing has over the lifetime of the listing.<br><br>Psuedocoe/~SQL:<br><br>IF scrape_date - first_review &lt;= 30 THEN number_of_reviews<br>ELSE number_of_reviews / ((scrape_date - first_review + 1) / (365/12)) |
...
| `estimated_revenue_l365d` | real | Estimated revenue in U.S. dollars generated by the listing over the last 365 days, computed using Inside Airbnb's "San Francisco Model" (`price` * estimated booked nights, based on `number_of_reviews_ltm`, `minimum_nights`, and `availability_365`). |
| `amenities_count` | integer | Number of items in each listing's `amenities` list, computed for this course. |
